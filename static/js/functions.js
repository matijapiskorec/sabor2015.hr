    ...
    link: function (scope, element, attrs) {
     ...
     scope.$watchGroup(['election_regions','friends_data'], function (newData, oldData) {
        ...
        function changeAnimated(data,duration) {
          ...
          // var sumVotes = function(data) {
          //   return data.map(function(d){return d.votes})
          //              .reduce(function(prev,curr){return curr + prev;})
          //     };

          // var counter = d3.select("#total-votes-friends")
          //   .data(sumVotes(was));

          // counter = d3.select("#total-votes-friends")
          //   .data(sumVotes(is));

          // counter.transition().duration(duration)
          //   .tween("text",
          //     function (d) {
